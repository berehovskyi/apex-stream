...
public class AddOperator extends Operator {
    ...
    public AddOperator(Long l) { this.l = l; }
...
Returns lazily concatenated `LongEnumerable` from `iterables`.
...
Long sum = LongStream.of(new List<Long>{ 0, 5L, 1L, -10 }).fold(0L, new SumBiOperator()); // -4L
Long factorialOfN = LongStream.range(1, n).fold(1, new ProductBiOperator());
```
...
###### Example
```apex
Set<Long> longs = LongStream.of(new List<Long>{ 0L, 5L, 1L, -10L })
    .skip(1)
    .toSet(); // [5L, 1L, -10L, 0L]
```
